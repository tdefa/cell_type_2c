...
from neural_subgraph_learning_GNN.common import combined_syn
from neural_subgraph_learning_GNN.subgraph_mining.config import parse_decoder
from neural_subgraph_learning_GNN.subgraph_matching.config import parse_encoder
from neural_subgraph_learning_GNN.subgraph_mining.search_agents import GreedySearchAgent, MCTSSearchAgent
...
def pattern_growth(dataset, task, args):
            ...
            for j in tqdm(range(args.n_neighborhoods)):
                graph, neigh = utils.sample_neigh(graphs,
                    random.randint(args.min_neighborhood_size,
                        args.max_neighborhood_size)) #sample a random subgraph
                neigh = graph.subgraph(neigh) # extraction of the subgraph
                neigh = nx.convert_node_labels_to_integers(neigh)
                neigh.add_edge(0, 0) #loop to define the place of the anchors?
                neighs.append(neigh)
                if args.node_anchored:
                    anchors.append(0)   # after converting labels, 0 will be anchor
    # so far neighs are the sample subgraph input for the following
    embs = []
    ...
    for i in range(len(neighs) // args.batch_size):
        ...
        with torch.no_grad():
            batch = utils.batch_nx_graphs(neighs[i*args.batch_size:top],
                anchors=anchors if args.node_anchored else None) # add feature to node for the embedding
            emb = model.emb_model(batch) # USE THE GNN to emb all the sample (batched) subgraph
    ...
    for pattern in out_graphs:
        ...
        print(args.dataset)
        print("Saving plots/cluster/" + args.dataset + "{}-{}.png".format(len(pattern),
            count_by_size[len(pattern)]))
        plt.savefig("plots/cluster/" + args.dataset + "{}-{}.png".format(len(pattern),
            count_by_size[len(pattern)]))
        plt.savefig("plots/cluster/" + args.dataset + "{}-{}.pdf".format(len(pattern),
            count_by_size[len(pattern)]))
        plt.close()
    ...
    if not os.path.exists("results"):
        os.makedirs("results")
    with open(args.out_path[:-2] + args.dataset + '.p', "wb") as f:
...
def main():
    ...
    args.dataset = 'lung_graph' #"enzymes" #
    args.path_to_lung = "/home/tom/Bureau/annotation/cell_type_annotation/to_take/201030_fridyay/adjacent_list/"
    print(args)
    ...
    elif args.dataset == 'lung_graph':
        files_list_lung = glob.glob(
            args.path_to_lung  + "ad_listdd_t5_2907*")
        graph = nx.Graph()
        for f in files_list_lung[:1]:
            print(f)
            ad_list = np.load(f)
            for edge in ad_list:
                    graph.add_edge(int(edge[0]), int(edge[1]))
        print("is directed %s " % str(nx.is_directed(graph)))
