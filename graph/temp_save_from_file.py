...
# https://github.com/ColinConduff/FrequentSubgraphMining/blob/master/gaston_py/graph.py
    ...
    with open(file_path, "w") as f:
        for g_id, graph in enumerate(graphs):
            ...
            for index, (node_id, data) in enumerate(graph.nodes(data=True)):
                ...
                node_label = 1 #data['label']
                f.write("v {} {}\n".format(index, node_label))

            for source, target, data in graph.edges(data=True):
                f.write("e {} {} {}\n".format(node_dict[source], node_dict[target], 1))
...
def count_unique_nodes(graphs):
    return len(set(data['label'] for graph in graphs for _, data in graph.nodes(data=True)))
